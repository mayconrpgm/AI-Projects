...
class Network(nn.Module):
    ...
    def __init__(self, inputSize, numberOfActions):
        ...
        numberOfHiddenNeurons1 = 50
        numberOfHiddenNeurons2 = 30
        numberOfHiddenNeurons3 = 30
        #Estabilishing the connection between the input layer and the hidden layer
        self.fullConnection1 = nn.Linear(inputSize, numberOfHiddenNeurons1)
        #Estabilishing the connection between the hidden layer and the output layer
        self.fullConnection2 = nn.Linear(numberOfHiddenNeurons1, numberOfHiddenNeurons2)
        self.fullConnection3 = nn.Linear(numberOfHiddenNeurons2, numberOfHiddenNeurons3)
        self.fullConnection4 = nn.Linear(numberOfHiddenNeurons3, numberOfActions)
        
    def forward(self, state):
        #F.relu: the rectified linear unit activation function (https://qph.ec.quoracdn.net/main-qimg-b0a1423dbff251a5d46117cc72943d2b)
        x = F.relu(self.fullConnection1(state))
        y = F.relu(self.fullConnection2(x))
        z = F.relu(self.fullConnection3(y))
        qValues = self.fullConnection4(z)
        return qValues
